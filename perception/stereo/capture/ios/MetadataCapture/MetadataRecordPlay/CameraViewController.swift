...
class CameraViewController: UIViewController, AVCaptureFileOutputRecordingDelegate, CLLocationManagerDelegate {
		...
		if session.canAddOutput(movieFileOutput) {
			...
			if let movieFileOutputVideoConnection = movieFileOutput.connection(withMediaType: AVMediaTypeVideo) {
				...
				if movieFileOutputVideoConnection.isVideoStabilizationSupported {
					movieFileOutputVideoConnection.preferredVideoStabilizationMode = .cinematic
				}
	...
	func capture(_ captureOutput: AVCaptureFileOutput!, didFinishRecordingToOutputFileAt outputFileURL: URL!, fromConnections connections: [Any]!, error: Error!) {
		...
		*/
		...
		if error != nil {
			print("Movie file finishing error: \(error.localizedDescription)")
	...
	private func connectMetadataPorts() {
		...
		if !isConnectionActiveWithInputPort(AVMetadataIdentifierQuickTimeMetadataDirectionMotion) {
			connectSpecificMetadataPort(AVMetadataIdentifierQuickTimeMetadataDirectionMotion)
		}
	}
